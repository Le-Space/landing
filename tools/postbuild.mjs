...
// Registration is inline so it does not depend on the app bundle loading.
...
for (const pagePath of PAGES) {
  ...
  for (const lang of LOCALES) {
    ...
    const head = [
      ...
      // Registered from the page rather than the app bundle, so the site keeps
      // working offline even if the bundle itself fails to load.
      SW_REGISTRATION,
...
// --- Service worker -------------------------------------------------------
//
// Generated here, not during the vite build: postbuild rewrites every page
// afterwards, so a precache list built earlier would name files that no longer
// exist — a cache pinning stale pages, which is worse than no cache because
// nobody notices.
//
// Strategy, deliberately conservative:
//   • navigations  → network first, cache as fallback. A deploy is visible on
//     the next load, and a dead gateway still serves the last good page.
//   • /assets/…    → cache first. Those carry a content hash; they never change
//     under the same name.
//   • everything else same-origin → cache first, network fills the gap.
// Anything from another origin is left alone entirely.
//
// A wrong service worker is the one bug a deploy cannot fix, because old
// visitors keep the old worker. Hence network-first for HTML, always.
const PRECACHE_EXT = new Set(['.html', '.js', '.css', '.woff2', '.svg', '.ico', '.webmanifest']);

async function collectPrecache(dir, base = '') {
  const out = [];
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    const rel = `${base}/${entry.name}`;
    if (entry.isDirectory()) {
      out.push(...(await collectPrecache(join(dir, entry.name), rel)));
    } else if (PRECACHE_EXT.has(extname(entry.name))) {
      // index.html is reachable as its directory; caching both wastes a slot.
      out.push(entry.name === 'index.html' ? `${base}/` || '/' : rel);
    }
  }
  return out;
}

const precache = [...new Set(await collectPrecache(dist))].sort();
const swVersion = `${site}-${process.env.APP_COMMIT ?? process.env.GITHUB_SHA ?? 'dev'}`.slice(0, 40);

await writeFile(
  resolve(dist, 'sw.js'),
  `// Generated by tools/postbuild.mjs — do not edit.
...
// Drop every cache that is not this build's. Without this each deploy leaves
// another complete copy behind.
...
console.log(`${site}: sw.js → ${precache.length} Dateien vorab, Version ${swVersion}`);
